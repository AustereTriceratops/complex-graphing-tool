...
test('test initial minus signs', () => {
    ...
    // tokens = preParser(tokens);
    // expectedTokens = [X, POW, LPAREN, NUM, MINUS, NUM, RPAREN, END]

    // tokens.map((t, i) => expect(t.name).toEqual(expectedTokens[i]));

    // tokens = Lexer.scan("x * -1");
    // tokens = preParser(tokens);
    // expectedTokens = [X, TIMES, LPAREN, NUM, MINUS, NUM, RPAREN, END]

    // tokens.map((t, i) => expect(t.name).toEqual(expectedTokens[i]));

    // tokens = Lexer.scan("x / -1");
    // tokens = preParser(tokens);
    // expectedTokens = [X, DIVIDE, LPAREN, NUM, MINUS, NUM, RPAREN, END]
